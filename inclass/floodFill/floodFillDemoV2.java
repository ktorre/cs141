// This demo uses the flood fill method which checks all 8 adjacent cells.
public class floodFillDemoV2 {
	...
	display( C );
	pause( 200 );
	// These are the original 4 cells
	if ( x + 1 < C.length )
	...
	// Now we check the "corner" cells
	if ( x - 1 >= 0 && y - 1 >= 0 )
	    floodFill( x - 1, y - 1, c, C ); // g Up-Left
	if ( x + 1 < C.length && y - 1 >= 0 )
	    floodFill( x + 1, y - 1, c, C ); // go Up-Right
	if ( x - 1 >= 0 && y + 1 < C[ 0 ].length )
	    floodFill( x - 1, y + 1, c, C ); // go Down-Left
	if ( x + 1 < C.length && y + 1 < C[ 0 ].length )
	    floodFill( x + 1, y + 1, c, C ); // go Down-Right
    }
